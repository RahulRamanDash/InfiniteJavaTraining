...
public class BankDAOimpl implements BankDAO{
	...
	public String withdrawAmountDao(int userAccountNo,int withdrawamount) throws ClassNotFoundException, SQLException {
		...
		if(bank!=null) {
			...
			if(amount-withdrawamount>=1000) {
				connection = BankConnectionHelper.getConnection();
				String cmd = "Update Bank set amount=amount - ? where accountNo=? and status='active'";
				pst = connection.prepareStatement(cmd);
	...
	@Override
	public Bank searchAccountDao(String userName) throws ClassNotFoundException, SQLException {
		connection = BankConnectionHelper.getConnection();
		String cmd = "select * from Bank where firstName=?";
		pst = connection.prepareStatement(cmd);
