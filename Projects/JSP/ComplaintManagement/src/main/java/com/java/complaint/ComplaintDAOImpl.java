...
public class ComplaintDAOImpl implements ComplaintDAO{
	...
	public int calculateTAT(Timestamp complaintDate) {
	    if (complaintDate == null) {
	        throw new IllegalArgumentException("ComplaintDate cannot be null");
	    }
	    Timestamp endDate = new Timestamp(new Date().getTime());
	    long ms = endDate.getTime() - complaintDate.getTime();
	    int tat = (int) Math.ceil(ms / (1000.0 * 60 * 60 * 24));	    
	    return tat;
	}
	
	public int calculateTAT2inp(Timestamp complaintstDate,Timestamp complaintendDate) {
	    if (complaintstDate == null) {
	        throw new IllegalArgumentException("ComplaintDate cannot be null");
	    }
	    Timestamp endDate = new Timestamp(new Date().getTime());
	    long ms = complaintendDate.getTime() - complaintstDate.getTime();
	    int tat = (int) Math.ceil(ms / (1000.0 * 60 * 60 * 24));	    
	    return tat;
	}
